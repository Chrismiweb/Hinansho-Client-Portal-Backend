...
const resend = require("../middleware/resend");
...
const createInvestorByAdmin = async (req, res) => {
  try {
    ...
    // 5️⃣ Prepare logo as base64 (for inline CID)
    const logoPath = path.join(__dirname, "../Hinansho Gold 2 1.png");
...
<body style="background:#F5F7FB;font-family:Segoe UI,Arial,sans-serif;">
  <div style="max-width:600px;margin:40px auto;background:#fff;border-radius:14px;box-shadow:0 6px 25px rgba(0,0,0,.08);overflow:hidden">
    ...
    <div style="padding:28px;text-align:center;border-bottom:2px solid #C9A24D">
      <img src="content_id:logo" width="90" />
      <h2>Hinansho Client Portal</h2>
    </div>
    ...
    console.log("Resend email response:", resendResponse);
...
const resendOTP = async (req, res) => {
  try {
    ...
    const { error } = await resend.emails.send({
      ...
      to: email,
      subject: "New Verification OTP - Hinansho Client Portal",
      html: `Your new verification OTP is <strong>${otp}</strong>. Valid for 10 minutes.`,
      attachments: [
        {
          filename: "logo.png",
          content: logoBase64,
...
const sendResetOtp = async (req, res) => {
  try {
    ...
    const { error } = await resend.emails.send({
      ...
      to: email,
      subject: "Password Reset Request - Hinansho Client Portal",
      html: `Your password reset OTP is <strong>${otp}</strong>. Valid for 10 minutes.`,
      attachments: [
        {
          filename: "logo.png",
          content: logoBase64,
...
const resendResetOtp = async (req, res) => {
  try {
    ...
    const { error } = await resend.emails.send({
      ...
      to: email,
      subject: "New Password Reset OTP - Hinansho Client Portal",
      html: `Your new password reset OTP is <strong>${newOtp}</strong>. Valid for 10 minutes.`,
      attachments: [
        {
          filename: "logo.png",
          content: logoBase64,
    ...
    console.error("Error in resendResetOtp:", error);
    res.status(500).json({
      error: "Failed to resend password reset OTP",
      success: false,
    });
  }
};
